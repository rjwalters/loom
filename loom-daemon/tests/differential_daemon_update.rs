...
//! | `$PATH` (and therefore the stale-entry-point scan) | [`FIXED_PATH`] + the fixture's own `bin/` |
...
/// The system half of the PATH both sides run under. The fixture's own `bin/`
/// is prepended, so the stale-entry-point scan sees exactly what the corpus
/// puts there and nothing the host happens to have installed.
...
fn materialise(case: &Case, root: &Path) -> (BTreeMap<String, String>, PathBuf, String) {
    ...
    env.insert("PATH".into(), format!("{}:{FIXED_PATH}", bin.display()));
...
fn run_case(case: &Case, prog: &Path, leading_args: &[String]) -> Answer {
    ...
    //
    // Replay does none of this: `leading_args` is empty for the port, so this
    // whole block is skipped and no `bash` is needed (the property the frozen
    // oracle exists to preserve).
    let mut leading: Vec<String> = leading_args.to_vec();
    if case.self_located && !leading.is_empty() {
