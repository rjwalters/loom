...
pub struct DaemonStatusReport {
    ...
    /// Live state of an in-progress drain-and-restart roll (Issue #8514):
    /// `roll_pending`, `started_at`, the elapsed paused-dispatch duration, the
    /// refusal count and the in-flight count it is waiting on. `None` when no
    /// drain is active — and from a pre-#8514 daemon binary, which never
    /// computed one. Unlike [`Self::drain_note`] (a one-shot note written at a
    ...
    /// advancing — which class of failure is responsible.
    ///
    /// `None` **only** from a pre-ADR-0021 daemon binary that never computed
    /// one. A daemon of this vintage with no `forgeEvents` block reports
    /// `Some(ForgeEventsStatus::disabled())`, so silence is never the answer.
    #[serde(default)]
    pub forge_events: Option<ForgeEventsStatus>,
...
mod roster_status;
pub use roster_status::{RosterMemberStatus, RosterStatus};
