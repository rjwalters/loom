        ...
        let final_content = if existed {
            ...
            if existing_content.contains(LOOM_SECTION_START) {
                ...
                if let (Some(start_idx), Some(end_idx)) = (
                    ...
                    // upgrade. Run the legacy heuristic on the SLICES here:
                    // `is_legacy_loom_managed_root` early-returns `false` on
                    // marker-containing content, so a whole-file check can
                    // never catch this case. When either slice is itself
                    // legacy-managed, replace the entire file with the
                    // wrapped pointer instead of preserving the legacy text.
                    if is_legacy_loom_managed_root(before) || is_legacy_loom_managed_root(after) {
