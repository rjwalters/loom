...
impl SweepRegistry {
    ...
    pub(crate) fn begin_prepared_issue_dispatch(
        ...
        depends_on: Option<u32>,
        resume_bypass_pr: Option<u32>,
        prepared_launch: Option<private_dispatch::PreparedLaunch>,
        ...
        let (admission, resolved_model, selection) = if let Some(prepared) = prepared_launch {
            (prepared.admission, prepared.model, prepared.selection)
        } else {
        ...
        if let Some(selection) = &selection {
            selection.rebind(&self.config.workspace_root, &sweep_id)?;
        }
...
pub(crate) fn dispatch_model_releasing_poll_lock(
    ...
    let prepared_launch = if tokio::runtime::Handle::try_current().is_ok() {
        tokio::task::block_in_place(|| {
            private_dispatch::prepare(registry, kind, idempotency_key.as_deref(), model)
        })?
    } else {
        private_dispatch::prepare(registry, kind, idempotency_key.as_deref(), model)?
    };
