...
pub(crate) enum DepRecheckCommand {
    ...
    /// churn never changes CONCLUSION_HASH. The merge-state bucket is `n/a` for
    /// any PR that is not OPEN (#8253): GitHub stops computing mergeability
    /// once a PR merges or closes, so a MERGED/CLOSED PR's `mergeable`/
    /// `mergeStateStatus` reading is meaningless and must not move the hash.
    DepRecheck {
        #[command(flatten)]
        common: CommonArgs,
