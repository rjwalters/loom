...
/// exactly like the reaper's own log lines already do.
...
pub fn reap_worktrees_only(repo_root: &Path, config: &WorktreeReaperConfig) -> ReapReport {
    ...
    //
    // #7590: routed through `remove_with_backoff` so a permanently-failing
    // path (a root-owned, permission-denied build-cache directory, or any
    // cause that repeats `REMOVAL_FAILURE_CAP` times running) stops being
    // retried every single tick and is surfaced on `loom-daemon health`
    // instead — see [`stuck_worktree_removals`].
    let remover = |path: &Path, issue: u32| {
        remove_with_backoff(repo_root, WorktreeKind::Issue, issue, path, || {
            clean::cleanup_worktree(repo_root, path, issue, false, "worktree_reaper").map(|_| ())
        })
    };
    ...
    // #7590: same shared backoff/tracking wrapper as the issue-keyed
    // `remover` above — a `pr-<N>` worktree is exactly as susceptible to a
    // root-owned, permission-denied nested build-cache directory.
    let pr_remover = |path: &Path, pr: u32| {
        let merged_head_sha = probe_pr(pr).head_sha;
        remove_with_backoff(repo_root, WorktreeKind::Pr, pr, path, || {
            clean::cleanup_pr_worktree(
                repo_root,
                path,
                pr,
                false,
