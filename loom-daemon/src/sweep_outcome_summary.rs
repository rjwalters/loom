...
mod tests {
    ...
    fn weighted_tokens_price_through_the_named_rate_card() {
        ...
        let expected = ModelPricing::for_model("claude-opus-5").calculate_cost(
            10_000,
            4_000,
            Some(5_000),
            Some(3_000),
        ) + ModelPricing::for_model("claude-haiku-4").calculate_cost(
            1_000,
            500,
            Some(0),
            Some(0),
    ...
    fn the_two_rate_cards_agree() {
        for (alias, pinned) in [
            ...
            let via_experiment =
                crate::script_helpers::sweep_experiment::model_pricing(Some(alias));
    ...
    fn merges_per_weighted_token_and_lines_per_merged_pr() {
        ...
        let usd =
            ModelPricing::for_model("claude-opus-5").calculate_cost(100_000, 0, Some(0), Some(0));
    ...
    fn two_workspace_fixture_aggregates_across_both_journals() {
        ...
        let mut opus =
            record("s-opus", "o/alpha", Some("claude-opus-5"), SweepResult::Success, 900);
        ...
        let mut dead =
            record("s-dead", "o/beta", Some("claude-sonnet-5"), SweepResult::Failure, 900);
