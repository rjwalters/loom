...
// ===================================================================
// refresh_local_workspace_state (issue #8121)
//
// `loom-daemon workspace add <path> --priority N` writes the machine
// registry and reports success immediately, but before #8121 the tick
// loop only reloaded that registry (and provisioned a new root's
// `SweepRegistry`/reaper/watchdog via `WorkspacePool::get_or_provision`)
// AFTER the rate-limit circuit breaker's early-`continue`, so a
// workspace registered while the breaker was suppressing gh polling
// would sit un-provisioned — invisible to `loom-daemon status` and
// dispatch — for the breaker's entire suppression window. These tests
// exercise the extracted `refresh_local_workspace_state` helper the
// tick loop now calls BEFORE that breaker check, proving the reload +
// provisioning is a pure, always-safe-to-call local operation that
// never needs to consult (or be gated by) the breaker at all.
// ===================================================================
