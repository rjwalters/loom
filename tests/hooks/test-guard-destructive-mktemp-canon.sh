...
# =========================================================================
#
# `NAME=$(mktemp -d)` followed by `NAME=$(cd "$NAME" && pwd -P)` (the routine
# way to resolve a symlinked temp root, e.g. macOS /tmp -> /private/tmp) used
# to trip the "two or more assignments poison the resolution" ambiguity rule
...
# the directory the first one already proved mktemp-safe: `cd` either fails
# (the substitution captures nothing, NAME becomes empty) or succeeds and
# `pwd -P` prints that same directory's canonical path.
#
...
# ---- rm-scope half: newly ALLOWED shapes -------------------------------
# The issue's own repro.
assert_allow_env "rm-scope (#7986): mktemp -d then \$(cd \"\$VAR\" && pwd -P) allows (issue repro)" \
...
assert_allow_env "rm-scope (#7986): both assignments double-quoted allows" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT="$(mktemp -d)"; TMPROOT="$(cd "$TMPROOT" && pwd -P)"; rm -rf "$TMPROOT"' "$REPO_ROOT"
assert_allow_env "rm-scope (#7986): \$(realpath \"\$VAR\") canonicalization form allows" \
...
assert_allow_env "rm-scope (#7986): bare mktemp (file, no -d) then canonicalization allows" \
    "LOOM_RM_SCOPE=repo" 'F=$(mktemp); F=$(cd "$F" && pwd -P); rm -f "$F"' "$REPO_ROOT"
...
assert_deny_env "rm-scope (#7986): canonicalizing a DIFFERENT variable still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d); TMPROOT=$(cd "$OTHER" && pwd -P); rm -rf "$TMPROOT"' "$REPO_ROOT"
...
assert_deny_env "rm-scope (#7986): a third assignment after the safe chain still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d); TMPROOT=$(cd "$TMPROOT" && pwd -P); TMPROOT=/opt/vendor/important; rm -rf "$TMPROOT"' "$REPO_ROOT"
...
assert_deny_env "rm-scope (#7986): reversed order (canonicalize then mktemp) still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(cd "$TMPROOT" && pwd -P); TMPROOT=$(mktemp -d); rm -rf "$TMPROOT"' "$REPO_ROOT"
# `pwd` without -P is a different string -- exact match or nothing.
assert_deny_env "rm-scope (#7986): 'pwd' without -P is not the admitted form, still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d); TMPROOT=$(cd "$TMPROOT" && pwd); rm -rf "$TMPROOT"' "$REPO_ROOT"
# `||` instead of `&&` inverts the meaning: on a SUCCESSFUL cd it captures
# nothing, and on failure it prints the CURRENT directory -- must never match.
assert_deny_env "rm-scope (#7986): '||' instead of '&&' still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d); TMPROOT=$(cd "$TMPROOT" || pwd -P); rm -rf "$TMPROOT"' "$REPO_ROOT"
...
assert_deny_env "rm-scope (#7986): canonicalization with a literal prefix on the RHS still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d); TMPROOT=/etc$(cd "$TMPROOT" && pwd -P); rm -rf "$TMPROOT"' "$REPO_ROOT"
assert_deny_env "rm-scope (#7986): canonicalization with a literal suffix on the RHS still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d); TMPROOT=$(cd "$TMPROOT" && pwd -P)/sub; rm -rf "$TMPROOT"' "$REPO_ROOT"
# The brace form is deliberately NOT admitted (narrowness over convenience).
assert_deny_env "rm-scope (#7986): \${VAR} brace form inside cd is not admitted, still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d); TMPROOT=$(cd "${TMPROOT}" && pwd -P); rm -rf "$TMPROOT"' "$REPO_ROOT"
...
assert_deny_env "rm-scope (#7986): custom-template mktemp + canonicalization still denies" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d /opt/other/XXXXXX); TMPROOT=$(cd "$TMPROOT" && pwd -P); rm -rf "$TMPROOT"' "$REPO_ROOT"
...
assert_deny_env "rm-scope (#7986): an unrelated rm target is unaffected by a proven chain" \
    "LOOM_RM_SCOPE=repo" 'TMPROOT=$(mktemp -d); TMPROOT=$(cd "$TMPROOT" && pwd -P); rm -rf "$SOMETHINGELSE7986"' "$REPO_ROOT"
...
# canonicalized, written into, then torn down).
assert_allow "write-confinement (#7986): mkdir -p under a canonicalized mktemp dir -> allow (issue repro)" \
    'TMPROOT=$(mktemp -d); TMPROOT=$(cd "$TMPROOT" && pwd -P); mkdir -p "$TMPROOT/sub"; rm -rf "$TMPROOT"' "$WT_REPO"
...
assert_allow "write-confinement (#7986): tee target under a canonicalized mktemp dir -> allow" \
    'tmp=$(mktemp -d); tmp=$(cd "$tmp" && pwd -P); echo x | tee "$tmp/out.txt"' "$WT_REPO"
assert_allow "write-confinement (#7986): cp destination under a canonicalized mktemp dir -> allow" \
    'tmp="$(mktemp -d)"; tmp="$(cd "$tmp" && pwd -P)"; cp /tmp/a.sh "$tmp/out.sh"' "$WT_REPO"
assert_allow "write-confinement (#7986): \$(realpath \"\$VAR\") canonicalization form -> allow" \
...
assert_allow "write-confinement (#7986): bare \$VAR target (no suffix) after the chain -> allow" \
    'TMPFILE=$(mktemp); TMPFILE=$(cd "$TMPFILE" && pwd -P); echo hi > "$TMPFILE"' "$WT_REPO"
...
assert_deny "write-confinement (#7986): canonicalizing a DIFFERENT variable -> denies" \
    'tmp=$(mktemp -d); tmp=$(cd "$other" && pwd -P); echo x > "$tmp/out.txt"' "$WT_REPO"
assert_deny "write-confinement (#7986): a third assignment after the safe chain -> denies" \
    'tmp=$(mktemp -d); tmp=$(cd "$tmp" && pwd -P); tmp=/some/other/path; echo x > "$tmp/out.txt"' "$WT_REPO"
assert_deny "write-confinement (#7986): reversed order (canonicalize then mktemp) -> denies" \
    'tmp=$(cd "$tmp" && pwd -P); tmp=$(mktemp -d); echo x > "$tmp/out.txt"' "$WT_REPO"
assert_deny "write-confinement (#7986): 'pwd' without -P -> denies" \
    'tmp=$(mktemp -d); tmp=$(cd "$tmp" && pwd); echo x > "$tmp/out.txt"' "$WT_REPO"
assert_deny "write-confinement (#7986): custom-template mktemp + canonicalization -> denies" \
    'tmp=$(mktemp -d --tmpdir=/other/dir); tmp=$(cd "$tmp" && pwd -P); echo x > "$tmp/out.txt"' "$WT_REPO"
...
assert_deny "write-confinement (#7986): '..' traversal in the suffix after a proven chain -> denies" \
    'tmp=$(mktemp -d); tmp=$(cd "$tmp" && pwd -P); cp /tmp/a.sh "$tmp/../../evil.sh"' "$WT_REPO"
assert_deny "write-confinement (#7986): an unrelated unresolved \$VAR write is unaffected by a proven chain -> denies" \
    'tmp=$(mktemp -d); tmp=$(cd "$tmp" && pwd -P); echo pwned > "$OTHERVAR7986/evil.sh"' "$WT_REPO"
assert_deny "write-confinement (#7986): a write resolving INSIDE the main checkout still denies" \
    "tmp=\$(mktemp -d); tmp=\$(cd \"\$tmp\" && pwd -P); echo pwned > $WT_REPO/defaults/hooks/evil.sh" "$WT_REPO"
