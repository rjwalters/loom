...
| Both credentials appear only as `${VAR:?…}` required interpolations, in the casting, the lock and the render | A committed literal is a leaked secret; a non-required interpolation starts the stack with an empty signing secret |
