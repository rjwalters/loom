...
The `loom:operator-only` **label** is the one hard exclusion the "Aggressive candidate taxonomy" table enforces for the `all` sentinel (and the one hard skip Modes A/B/C already apply outside the sentinel). In practice, some issues declare operator-gating in **body text** without ever getting that label — "this acquisition step is operator-gated", "Operator decision: hold for credentials", "requires operator authorization" — and every mode would otherwise silently plan them for automated build. This section adds a **second, advisory-only** signal layered on top of the label check: a phrase scan of the same `body` already fetched for "Overlap-aware wave partitioning" and `--auto-stack` edge detection (Stage 0 candidate survey step 1 / dry-run survey step 1, "Add `body` to this read unconditionally") — **no extra API call**.

**Advisory only — never a hard skip, never a label mutation, never blocking.** Unlike the `loom:operator-only` label row (a hard `Skip`), a body-text match here changes nothing about the candidate's planned action: it still shows its normal `would build` (or whatever the label routing already decided) — annotated with a `⚠` warning so the **operator** can decide at the confirmation gate that already exists for the issue-set path (Mode B's candidate-set gate and the `all` sentinel's mandatory confirmation gate). Nothing here removes a label, closes an issue, or skips a candidate — only a human decision (or a follow-up `loom:operator-only` label applied by a human) does that.

**1. Phrase scan — instruction-shaped fragments, not bare substrings.** Case-insensitive match against `body`, mirroring the `loom:blocked` row's hold/defer-phrase discipline (#4505 — instruction-shaped fragments, not a naive substring search) rather than flagging on a bare word that appears constantly in ordinary engineering prose (a bare `operator` or `credentials` substring would false-positive on "operator precedence" or "rotate credentials in CI"). The vocabulary:

- `operator-gated` (and the unhyphenated `operator gated`)
...
- `operator decision:`
- `login-walled` (and the unhyphenated `login walled`)
- `paid gpu`
- `requires credentials` (deliberately **not** the bare word `credentials` alone — see the false-positive rationale above)

A candidate whose body matches ≥1 fragment is flagged with the **first** matched phrase (quoted verbatim from the body, trimmed to a reasonable snippet) — dedup to one annotation per candidate even when multiple fragments match.

**2. Dependency-declared operator-gating (the `#87 → #4` shape).** Grep the same `body` for an in-set `Depends on #A` / `Requires #A` reference, reusing the exact `(Depends on|Requires)[*_:[:space:]]*#[0-9]+` vocabulary the `--auto-stack` edge-detection pass and `warn-out-of-set-deps.sh` already use (see "Auto-stack detection and wave ordering" / "Out-of-set dependency detect-and-warn") — but run this lookup **unconditionally**, independent of whether `--auto-stack` was passed. This check only *reads* the reference to look up `#A`'s labels; it never populates `DEPENDS_ON[N]` or creates a stacking edge, so it is exempt from the `--auto-stack` flag gating that scopes actual stack *topology* (#3729 kept that opt-in). If `#A` is itself a member of this run's resolved candidate set carrying the `loom:operator-only` **label** (its labels were already fetched by this same Stage 0 survey loop for `#A`'s own row — no extra lookup), flag the **child** too: its declared dependency cannot itself be completed by automation, so building the child now would build against a base nobody can finish. Under the `all` sentinel this fires routinely, since `all` resolves *every* open issue as a candidate — an in-set `#A` is the common case, not the exception. An out-of-set `#A` (not itself a candidate in this run) is left entirely to the existing "Out-of-set dependency detect-and-warn" pass, unchanged — this dependency-operator-gate check only fires when `#A`'s labels are already on hand from this same survey, keeping the "no extra API call" property intact.

**3. Surfacing — confirmation-gate annotation, not a new block.** Print the matched phrase inline, appended to the candidate's normal planned action, in the confirmation-gate listing (Mode B and the `all` sentinel's mandatory gate) and in the `--dry-run` plan's per-candidate line:

```
#87  "Acquire dataset via partner API"   labels: loom:issue   → would build  ⚠ body declares operator-gating: "acquisition is operator-gated"
#4   "Provision paid GPU instance"       labels: loom:issue   → would build  ⚠ body declares operator-gating: "paid GPU"
```

For the dependency case (`#87` depends on an in-set, operator-only `#4`):

```
#87  "Acquire dataset via partner API"   labels: loom:issue   → would build  ⚠ depends on #4 (loom:operator-only)
```

**4. Zero matches ⇒ byte-for-byte unchanged.** When no candidate's body matches any fragment and no candidate depends on an in-set `loom:operator-only` issue, no `⚠` annotation is emitted anywhere and the candidate-set / `--dry-run` output is identical to a run with this scan absent — this section adds no new "found nothing" line, exactly like the "Overlap analysis" and "Detected stacking pairs" blocks print nothing on a zero-match run.

**5. `--dry-run` composes.** The `--dry-run` plan (Modes A/B, "Issue-set output spec") shows the same `⚠` annotations on the same per-candidate line the confirmation gate does — this is a **read-only** scan (label reads already on hand + a body regex), so it runs identically whether or not `--dry-run` was supplied; nothing here is gated behind the dry-run/mutation boundary the way the orphaned-claim recovery pass is.

**Scope.** This scan runs over the issue-set survey (Modes A/B, and the `all` sentinel's issue path) wherever `body` is already fetched. Mode C (PR-set) is unaffected — a PR's own `loom:operator-only` label check (C0) is unchanged.
...
- **Operator-gate advisory annotation (optional, present only when the "Operator-gate advisory scan" matched, #5137)** — appended to the planned action on the same line, e.g. `would build  ⚠ body declares operator-gating: "acquisition is operator-gated"` or `would build  ⚠ depends on #4 (loom:operator-only)`. Never changes the planned action itself; absent when the scan found no match (see that section's "Zero matches ⇒ byte-for-byte unchanged" rule).
...
**Operator-gate advisory annotation (only when the "Operator-gate advisory scan" matched ≥1 candidate, #5137).** Unlike the "Detected stacking pairs" and "Overlap analysis" blocks above (both printed as separate blocks above the wave listing), the operator-gate advisory is an **inline** annotation appended to the matching candidate's own line, wherever it lands in the wave listing:
...
When no candidate's body matches the phrase vocabulary and no candidate depends on an in-set `loom:operator-only` issue, **no** `⚠` annotation is printed anywhere — the plan is byte-for-byte identical to a run with the scan absent, exactly like the two block-shaped signals above on their own zero-match runs.
...
| Operator-gate advisory scan (body-phrase detection) | **Implemented (#5137)** | Advisory-only, never a hard skip: the Stage 0 candidate survey (Modes A/B, and the `all` sentinel's issue path) scans the already-fetched `body` for operator-gate phrasing ("operator-gated", "requires operator authorization", "paid GPU", etc., instruction-shaped fragments per #4505's discipline) and flags candidates whose declared in-set `Depends on #A`/`Requires #A` is itself `loom:operator-only`. Annotates the matching candidate's line at the confirmation gate and in `--dry-run` output with a `⚠` warning; zero matches ⇒ byte-for-byte unchanged output. See "Operator-gate advisory scan". |
