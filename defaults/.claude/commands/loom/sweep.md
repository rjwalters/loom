...
| [`sweep-run-hygiene.md`](sweep-run-hygiene.md) | ~158 | Stop conditions; the five advisory **pre-wave** checks (host sleep, main-branch freshness, outstanding quarantine stashes, stale/undocumented `loom:blocked`, silent PR queues); the sweep-child working-set contract; peer-sweep / legacy-daemon / role-runner coexistence. |
