...
- **This prose guardrail is not the only line of defense.** A mechanical `Stop`-hook backstop (`defaults/hooks/guard-background-subagents.sh`, issue #4257, coverage extended to background Bash tasks by #4389) blocks the turn from ending — once, per stop sequence — when it detects an unresolved dispatched Task subagent or an outstanding `run_in_background` Bash task in the transcript. See `defaults/docs/guard-hooks.md`'s "Background Subagent Stop Guard" section for how it works and how to verify it is wired in a given repo.
