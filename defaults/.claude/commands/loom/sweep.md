...
- **The daemon / process-spawn path is unaffected**, same as `MODEL_CREDITS_EXHAUSTED` — `claude-wrapper.sh` already treats this signature as `TOKEN_EXHAUSTED` (rotate the account, mark it exhausted) via the existing #5631 pattern; no change needed there.
