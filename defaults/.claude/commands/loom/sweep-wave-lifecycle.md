...
1. **Verify the issue is open and not already in flight.**
   ...
   - **Existing-PR probe (#3359, #3677).** The set of open PRs for issue `N` is the **union of two GitHub-computed sources** — no body-grep. Both are additive and deduped by PR number before routing:
     ...
     2. **Non-closing cross-reference PRs (timeline, #3677), phrase-confirmed (#6216).** PRs that reference `N` with a **non-closing** phrase (`Part of #N` / `Contributes to #N`, the #3599 partial-increment convention — see `defaults/roles/builder-pr.md`) never appear in `closedByPullRequestsReferences` by design, so probe the issue's timeline for `cross-referenced` events whose source is a PR:
        ...
        **Phrase filter (required, #6216).** A `cross-referenced` event fires for **any** mention of `#N` — not only a partial-increment phrase — so a same-repo, open timeline hit from source 2 is only a candidate. For each candidate, fetch its body once and keep it only if it contains a `Part of #N` / `Contributes to #N` reference to **this** issue, tolerant of markdown emphasis/colon between phrase and `#N` (the convention `parse_dependencies` uses in `defaults/.claude/commands/loom/guide.md`, #4508):
        ```bash
        # One extra call per surviving candidate (bounded, not N×M — see the
        # cost note below).
        gh pr view <pr_number> --json body --jq '.body // ""' \
          | grep -iqE "(Part of|Contributes to)[*_:[:space:]]*#N([^0-9]|$)" \
          && echo confirmed || echo bare-mention
        ```
        `([^0-9]|$)` stops `Part of #1234` matching `#123`. `loom-daemon forge check-open-pr N` filters identically (#8940); its timeline leg also takes closing keywords — redundant with source 1, but it keeps the #4123 guard closed when the closes-graph leg cannot answer.

        Discard any candidate whose body does not match — a bare mention is not a partial-increment linkage and **must not** count as an open linked PR (it never feeds the union below or produces the `skip (existing PR #X in flight)` line). A discarded candidate may optionally get an advisory line visibly distinct from that skip line — e.g. `note: #N mentioned by open PR #Y, linkage not confirmed` — visibility only; it never changes routing.

     **Union, then filter at the per-PR read (#6217).** Merge the two source lists and dedupe by PR number, passing **every** source-1 PR through unfiltered at this step — `gh issue view --json closedByPullRequestsReferences` exposes only `id, number, repository, url` per entry, **not `state`**, so a `select(.state == "OPEN")` applied here matches nothing and silently drops every row (the #6217 bug: the union step has no `state` field to filter on). Source 2 needs no further filtering here — its timeline read already filtered to `state == "open"` **and** the phrase filter above discarded every bare mention (#6216), so only phrase-confirmed open PRs reach the union. For **every** candidate PR from the union (both sources), fetch its live state, labels, and draft flag — this per-PR read is the first and only point in the pipeline where `state` is available for a source-1 PR (`isDraft` rides along in the **same** read, so the draft/no-actionable-label row below costs no extra call, #8160):
     ```bash
...
**Stacked-dependency gated path (`--depends-on`, #3729 v1; per-issue map generalization, `--auto-stack`, #3759).** This gate fires **only** when a parent is set for the issue being built — look it up in the per-issue map `DEPENDS_ON[N]` (which subsumes the historical single global `DEPENDS_ON`: for a single-issue `--depends-on <parent>` dispatch, `DEPENDS_ON[N]` is just that one entry; for an `--auto-stack` wave, it is one entry per detected child). When `DEPENDS_ON[N]` is unset for issue `N`, the two steps below are byte-for-byte the default behavior. When `DEPENDS_ON[N]=<parent>` is set, the builder for issue `N` must:
  ...
  - Open its PR against the parent branch: `gh pr create --base feature/issue-<parent> --label "loom:review-requested" --body "Closes #N ..."` (instead of the default base). The PR stays stacked on the parent until reconciliation, which now fires automatically when the parent merges (see "Stacked dependency (auto-reconciliation on parent merge)").
...
**Reconciliation now fires automatically on parent merge (v2 item 1, #3747).** After the parent merges to the default branch (as a merge commit, #9105), the child branch still carries the parent's original commits in its history. `merge-pr.sh` now reconciles child PRs automatically at its post-merge choke point (alongside the partial-increment label reset, before branch deletion): it discovers open child PRs via a **live forge query** (`gh pr list --base feature/issue-<parent>` — never the daemon registry, which is ephemeral and may not be running under a Champion cron or interactive merge), then splits safe/unsafe per child on the child **issue's** `loom:building` label (fresh, uncached `gh api` read):
