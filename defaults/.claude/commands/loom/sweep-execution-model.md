...
- **The daemon / process-spawn path rotates, it does not downgrade.** With no per-dispatch model knob, `claude-wrapper.sh` answers `MODEL_CREDITS_EXHAUSTED` by rotating the account. #8058 changed the mark's *scope*, not the remedy: it names the model class in flight, and `tokens select --model` skips only that class, so an Opus ceiling no longer starves Sonnet. Class-less marks still block the account; `loom-daemon health`/`status` now break the healthy count down per class, so `2/20 healthy` is distinguishable from a dead pool. See `defaults/docs/token-pool.md`.
