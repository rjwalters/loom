...
#                             posture as .loom/CLAUDE.md. EXCEPTION (#5559): the
#                             two-line `**Loom Version**` / `Last updated` header
#                             at the top of .loom/CLAUDE.md IS a narrow,
#                             targeted-field resync (resync_claude_md_version_header
#                             below, same package.json-style field-edit pattern as
#                             the workspace stub version below) — everything else
#                             in .loom/CLAUDE.md, and all of .loom/AGENTS.md, stays
#                             out of scope pending #4479.
...
# `.loom/CLAUDE.md` to pin the #5559 version-header restamp)
...
#
# .loom/CLAUDE.md — the full vendored guide — is explicitly OUT OF SCOPE for a
# whole-file resync (see the header comment above): it is repo-customized at
# install time and needs the managed-section-markers design (#4479) before a
# full-file overwrite would be safe. But its two-line header —
# `**Loom Version**: X.Y.Z` / `Last updated: YYYY-MM-DD` — is a plain template
# substitution (defaults/.loom/CLAUDE.md's `{{LOOM_VERSION}}` /
# `{{INSTALL_DATE}}` placeholders, filled once by loom-daemon's template
# substituter at install time, loom-daemon/src/init/templates.rs) that resync
# never revisited. Because resync DOES keep .loom/install-metadata.json's
# loom_version current (restamp_metadata below), the two drift apart after
# every resync that lands a version bump — and scripts/install-loom.sh's
# idempotency check prefers install-metadata.json, so it never notices and
# never re-triggers the template substitution either (#5559). This mirrors the
# package.json targeted-field-edit pattern above: a two-line header restamp,
# not a whole-file resync, so any other repo customization elsewhere in the
# guide is preserved untouched.
resync_claude_md_version_header() {
    local cm="$REPO_ROOT/.loom/CLAUDE.md"
    # Pre-#4239 layout (no vendored guide yet), or a repo that never got one:
    # nothing to restamp.
    [[ -f "$cm" ]] || return 0
    ...
    local version today current_version current_date
    version="$(read_source_version)"
    today="$(date +%Y-%m-%d)"
    current_version="$(sed -n 's/^\*\*Loom Version\*\*: *//p' "$cm" | head -1)"
    current_date="$(sed -n 's/^Last updated: *//p' "$cm" | head -1)"

    if [[ "$current_version" == "$version" && "$current_date" == "$today" ]]; then
        note "  ${GREEN}unchanged${NC} .loom/CLAUDE.md (version header already current)"
        N_UNCHANGED=$((N_UNCHANGED + 1))
        return 0
    fi

    if [[ "$DRY_RUN" -eq 1 ]]; then
        printf '%b\n' "  ${BOLD}would update${NC} .loom/CLAUDE.md (restamp version header: ${current_version:-?} -> $version, ${current_date:-?} -> $today, #5559)"
        N_UPDATED=$((N_UPDATED + 1))
        return 0
    fi
    ...
    if sed -e "s/^\*\*Loom Version\*\*: .*/**Loom Version**: $version/" \
           ...
           "$cm" > "$tmp" 2>/dev/null && [[ -s "$tmp" ]]; then
        mv "$tmp" "$cm"
        printf '%b\n' "  ${GREEN}updated${NC}   .loom/CLAUDE.md (restamped version header: ${current_version:-?} -> $version, ${current_date:-?} -> $today, #5559)"
        N_UPDATED=$((N_UPDATED + 1))
    else
        rm -f "$tmp"
