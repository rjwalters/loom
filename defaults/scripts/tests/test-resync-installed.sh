...
# Same stub strategy as the label-check groups above (#7761): the real
# check-guards-installed.sh has its own suite
# (test-check-guards-installed.sh), so these groups install a scriptable STUB
# as defaults/scripts/check-guards-installed.sh and assert only on the WIRING
...
echo "Test group 29: guard-hook install check -- healthy install (#7761)"
REPO="$(make_fixture)"
add_guardcheck_stub "$REPO"
GUARD_STUB_LOG="$WORKDIR/guard29.log"; : > "$GUARD_STUB_LOG"
...
echo "Test group 29b: guard-hook install check -- broken, --fix repairs it (#7761)"
REPO="$(make_fixture)"
add_guardcheck_stub "$REPO"
GUARD_STUB_LOG="$WORKDIR/guard29b.log"; : > "$GUARD_STUB_LOG"
...
echo "Test group 29c: guard-hook install check -- unrepairable -> exit 75 (#7761)"
REPO="$(make_fixture)"
add_guardcheck_stub "$REPO"
GUARD_STUB_LOG="$WORKDIR/guard29c.log"; : > "$GUARD_STUB_LOG"
...
echo "Test group 29d: guard-hook install check -- --dry-run never tries to fix (#7761)"
REPO="$(make_fixture)"
add_guardcheck_stub "$REPO"
GUARD_STUB_LOG="$WORKDIR/guard29d.log"; : > "$GUARD_STUB_LOG"
