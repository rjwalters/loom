...
# Targeted field edits (never a whole-file resync):
#   (#4285) loom-workspace package.json decoy "version" field removed in place,
#           a consumer's own package.json/customized scripts block untouched,
#           .loom/resync-ignore pins it
#   (#5559) .loom/CLAUDE.md's `**Loom Version**` / `Last updated` header lines
#           restamped to match the resolved source -- the top
#           `**Installation Date**` line and the rest of the guide are left
#           untouched, --dry-run previews without writing, .loom/resync-ignore
#           pins it, and a repo with no vendored .loom/CLAUDE.md is a no-op
...
make_fixture() {
    ...
    # Vendored guide with a stale version header (#5559): drifts the same way
    # install-metadata.json does above, but resync must restamp only the
    # `**Loom Version**` / `Last updated` lines -- everything else (including
    # the top `**Installation Date**` line, which records the ORIGINAL install
    # date and must never change on resync) is a repo-owned customization that
...
# --- (#5559) targeted .loom/CLAUDE.md version-header restamp ----------------
echo "Test group 12g: .loom/CLAUDE.md version header restamped to match the source (#5559)"
REPO="$(make_fixture)"
OUT="$(cd "$REPO" && bash "$SCRIPT" 2>&1)"
RC=$?
CM="$REPO/.loom/CLAUDE.md"
if [[ $RC -eq 0 ]]; then pass "(#5559) apply with a stale CLAUDE.md header exits 0"; else fail "(#5559) apply exits 0 (got $RC)"; fi
if grep -q '^\*\*Loom Version\*\*: 9\.9\.9$' "$CM"; then
    pass "(#5559) **Loom Version** line restamped to the source version"
else
    fail "(#5559) **Loom Version** line NOT restamped"
fi
if grep -q "^Last updated: $(date +%Y-%m-%d)\$" "$CM"; then
    pass "(#5559) Last updated line restamped to today"
else
    fail "(#5559) Last updated line NOT restamped to today"
fi
if grep -q '^\*\*Installation Date\*\*: 2020-01-01$' "$CM"; then
    pass "(#5559) **Installation Date** line (the ORIGINAL install date) left untouched"
else
    fail "(#5559) **Installation Date** line was incorrectly modified"
fi
if grep -q "CONSUMER-CUSTOMIZED-SECTION" "$CM"; then
    pass "(#5559) rest of the consumer's guide content preserved"
else
    fail "(#5559) consumer's guide content was lost -- this must be a targeted field edit, not a whole-file resync"
fi
if grep -q "CLAUDE.md.*restamped version header" <<<"$OUT"; then
    pass "(#5559) apply reports the CLAUDE.md header restamp"
else
    fail "(#5559) apply did not report the CLAUDE.md header restamp"
fi
# Idempotent rerun: second apply is a clean no-op for the header.
OUT="$(cd "$REPO" && bash "$SCRIPT" 2>&1)"
if grep -q "CLAUDE.md (version header already current)" <<<"$OUT"; then
    pass "(#5559) second run reports CLAUDE.md header unchanged (idempotent)"
else
    fail "(#5559) second run did not report CLAUDE.md header as unchanged"
fi

echo "Test group 12h: --dry-run previews the CLAUDE.md header restamp without writing"
REPO="$(make_fixture)"
OUT="$(cd "$REPO" && bash "$SCRIPT" --dry-run 2>&1)"
RC=$?
if [[ $RC -eq 2 ]]; then
    pass "(#5559) --dry-run with a stale CLAUDE.md header exits 2"
else
    fail "(#5559) --dry-run exits 2 (got $RC)"
fi
if grep -q '^\*\*Loom Version\*\*: 0\.0\.0$' "$REPO/.loom/CLAUDE.md"; then
    pass "(#5559) --dry-run left the CLAUDE.md header UNCHANGED"
else
    fail "(#5559) --dry-run modified the CLAUDE.md header"
fi
if grep -q "would update.*CLAUDE.md.*restamp version header" <<<"$OUT"; then
    pass "(#5559) --dry-run reports it would restamp the CLAUDE.md header"
else
    fail "(#5559) --dry-run did not report the pending CLAUDE.md header restamp"
fi

echo "Test group 12i: .loom/resync-ignore pins .loom/CLAUDE.md against the header restamp"
REPO="$(make_fixture)"
printf '.loom/CLAUDE.md  # keep my pinned header\n' > "$REPO/.loom/resync-ignore"
OUT="$(cd "$REPO" && bash "$SCRIPT" 2>&1)"
if grep -q '^\*\*Loom Version\*\*: 0\.0\.0$' "$REPO/.loom/CLAUDE.md"; then
    pass "(#5559) pinned CLAUDE.md header NOT restamped"
else
    fail "(#5559) pinned CLAUDE.md header was restamped despite resync-ignore"
fi
if grep -q "skipped.*\.loom/CLAUDE\.md" <<<"$OUT"; then
    pass "(#5559) pinned CLAUDE.md reported as skipped"
else
    fail "(#5559) pinned CLAUDE.md not reported skipped"
fi

echo "Test group 12j: a repo with no vendored .loom/CLAUDE.md (pre-#4239 layout) is unaffected"
REPO="$(make_fixture)"
rm -f "$REPO/.loom/CLAUDE.md"
RC=0; OUT="$(cd "$REPO" && bash "$SCRIPT" 2>&1)" || RC=$?
if [[ $RC -eq 0 ]]; then
    pass "(#5559) apply still exits 0 when .loom/CLAUDE.md does not exist"
else
    fail "(#5559) apply exits 0 with no .loom/CLAUDE.md (got $RC)"
fi
if [[ ! -f "$REPO/.loom/CLAUDE.md" ]]; then
    pass "(#5559) no .loom/CLAUDE.md is created when the repo never had one"
else
    fail "(#5559) a .loom/CLAUDE.md was unexpectedly created"
fi
