...
echo ""
echo "Testing forge_text_has_unnegated_closing_ref (negation-aware closing-reference check, #1057)..."

if forge_text_has_unnegated_closing_ref "Closes #42" "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: plain 'Closes #N' is a real (unnegated) closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: plain 'Closes #N' should be a real closing reference"
fi

if forge_text_has_unnegated_closing_ref "Fixes #42" "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: plain 'Fixes #N' is a real (unnegated) closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: plain 'Fixes #N' should be a real closing reference"
fi

if forge_text_has_unnegated_closing_ref "Resolves #42" "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: plain 'Resolves #N' is a real (unnegated) closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: plain 'Resolves #N' should be a real closing reference"
fi

# The exact #1057 reproduction shape: PR #1051's body against #909.
...
if ! forge_text_has_unnegated_closing_ref "$PR1051_BODY" "909"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: exact #1057 repro ('**does not fix #909**', no other reference) is NOT a real closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: '**does not fix #909**' was wrongly treated as a real closing reference"
fi

if ! forge_text_has_unnegated_closing_ref "This change does not resolve #42 fully; a follow-up will." "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: 'not ... resolve #N' phrasing is NOT a real closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: 'not ... resolve #N' was wrongly treated as a real closing reference"
fi

if ! forge_text_has_unnegated_closing_ref "This doesn't fix #42 on its own." "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: contraction \"doesn't fix #N\" is NOT a real closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: \"doesn't fix #N\" was wrongly treated as a real closing reference"
fi

if ! forge_text_has_unnegated_closing_ref "We never fixed #42 in this pass." "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: 'never fixed #N' is NOT a real closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: 'never fixed #N' was wrongly treated as a real closing reference"
fi

# A negated reference to a DIFFERENT clause/sentence must not suppress a
# genuine, later close of the SAME issue elsewhere in the text.
if forge_text_has_unnegated_closing_ref "This does not fix #42 alone. Closes #42 together with the follow-up." "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: a genuine 'Closes #N' later in the text is NOT suppressed by an earlier negated mention of the same issue"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: a genuine later 'Closes #N' was wrongly suppressed by an earlier negated mention"
fi

# Non-closing keywords (`Updates #N`, `See #N`) still carry no closing
# reference at all -- not merely a negated one.
if ! forge_text_has_unnegated_closing_ref "Updates #42 with more detail." "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: 'Updates #N' (no closing keyword at all) is NOT a closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: 'Updates #N' was wrongly treated as a closing reference"
fi

if ! forge_text_has_unnegated_closing_ref "" "42"; then
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_PASSED=$((TESTS_PASSED + 1))
    echo -e "  ${GREEN}PASS${NC}: empty text has no closing reference"
else
    TESTS_RUN=$((TESTS_RUN + 1)); TESTS_FAILED=$((TESTS_FAILED + 1))
    echo -e "  ${RED}FAIL${NC}: empty text was wrongly treated as carrying a closing reference"
fi

# --- Test forge_pr_close_targets (Gitea): negation filtering end-to-end (#1057) ---
# The Gitea fallback regex has no GraphQL layer to defer to, so it must filter
# negated-only references itself (forge-helpers.sh's Gitea branch) rather than
# relying solely on a caller-side cross-check.
echo ""
echo "Testing forge_pr_close_targets (Gitea) end-to-end negation filtering (#1057)..."

FORGE_TYPE="gitea"
forge_get_repo_nwo() { echo "owner/repo"; }
forge_get_pr_body() { printf '%s' "$PR1051_BODY

Closes #42"; }

result=$(forge_pr_close_targets "999" | tr '\n' ' ' | sed 's/ $//')
assert_eq "42" "$result" "Gitea forge_pr_close_targets excludes a negated-only reference (#909) while still closing a real one (#42)"
