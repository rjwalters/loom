...
# forge (same `loom-daemon forge` / `gh` convention as
# `_worktree_merged_pr_head_sha` above). Unlike that helper, this one is used
# to decide whether it is SAFE to fall through to creating a brand-new branch
# named <branch> off the base ref — so, per #7765, "could not determine" must
# NOT be treated the same as "confirmed none exists": the whole defect this
# closes is a silent fresh main-HEAD branch shadowing a real PR (same-repo,
# unfetched — #4823 — or a fork's cross-repo head), so the caller must be able
# to tell "checked, no PR" apart from "could not check". MUST be called as a
# plain statement, never inside `$(...)` (subshell would discard the globals):
...
_worktree_open_pr_for_branch() {
    ...
    # (this file already treats it that way for the sibling #5657 check, and
...
if git show-ref --verify --quiet "refs/heads/$BRANCH_NAME"; then
    ...
    if git show-ref --verify --quiet "refs/remotes/origin/$BRANCH_NAME"; then
        ...
        # ─── #7765: forge-aware guard against a shadowed cross-repo PR ─────
        # origin has no branch of this name (confirmed absent, or the fetch
        # above simply failed — $_WT_ORIGIN_FETCH_RESULT). Either way we are
        # about to fall through to creating a FRESH branch under this exact
        # name below, which is precisely the silent-divergence trap #7765
        # describes: a fork PR's head (e.g. turian/loom:feature/issue-7717)
        # never shows up as origin/feature/issue-7717, so ref-absence on
        # origin is NOT proof that no PR already claims this branch name.
        # Ask the forge directly before deciding.
        _worktree_open_pr_for_branch "$BRANCH_NAME"
        case "$_WT_OPEN_PR_STATUS" in
            found)
                if [[ "$_WT_OPEN_PR_IS_CROSS_REPO" == "true" ]]; then
                    ...
                    # origin/$BRANCH_NAME would NOT touch the fork's PR) - so
                    # refuse rather than proceed blind, naming the real head
                    # and how to reach it (per the AC: refuse OR fetch; refuse
                    # is the simpler, unambiguous choice here).
                    if [[ "$JSON_OUTPUT" == "true" ]]; then
                        ...
                        print_error "Open PR #${_WT_OPEN_PR_NUMBER} for '$BRANCH_NAME' already exists with its head on a FORK ($_WT_OPEN_PR_HEAD_REPO:$_WT_OPEN_PR_HEAD_REF) - refusing to create a same-named branch from $BASE_DISPLAY, which would silently shadow it instead of the real work."
                        echo "  PR: ${_WT_OPEN_PR_URL:-<no url>}"
                        echo "  To work on it, either fetch the PR's merge ref directly:"
                        echo "    git fetch origin refs/pull/${_WT_OPEN_PR_NUMBER}/head:$BRANCH_NAME"
                    ...
                    # Same-repo PR whose head simply wasn't fetched yet by the
                    # plain-name fetch above (it failed, or gh's --head match
                    # found it under a ref this repo hadn't seen) — this is
                    # exactly the #4823 in-flight-cycle case, just reachable
                    # even when that fetch didn't resolve it. Fetch it
                    # explicitly (by PR number, which always resolves
                    # regardless of why the plain-name fetch missed) and
                    # reuse it — never fall through to a fresh branch here.
                    if [[ "$JSON_OUTPUT" != "true" ]]; then
                        print_info "Open PR #${_WT_OPEN_PR_NUMBER} already exists for '$BRANCH_NAME' on origin (not yet fetched) - fetching and reusing it"
                    fi
                    git fetch origin "refs/pull/${_WT_OPEN_PR_NUMBER}/head:refs/remotes/origin/$BRANCH_NAME" 2>/dev/null || true
                    if git show-ref --verify --quiet "refs/remotes/origin/$BRANCH_NAME"; then
                        _WT_REUSE_REMOTE_BRANCH=true
                    else
                        # The forge confirmed the PR exists but we still
                        # couldn't materialize its ref locally - refuse rather
                        # than silently branching fresh under its name.
                        if [[ "$JSON_OUTPUT" == "true" ]]; then
                            echo '{"success": false, "error": "open-pr-ref-fetch-failed", "issueNumber": '"$ISSUE_NUMBER"', "prNumber": '"${_WT_OPEN_PR_NUMBER:-null}"'}' >&3
                        else
                            print_error "Open PR #${_WT_OPEN_PR_NUMBER} exists for '$BRANCH_NAME' but its ref could not be fetched from origin - refusing to create a same-named branch from $BASE_DISPLAY."
                        fi
                        exit 1
                    fi
                fi
            ...
            not_found|no_forge_remote)
                ...
                # clone) - there is no PR to shadow either way. Both are safe
                # to fall through to a fresh branch below.
                :
                ;;
            unavailable)
                ...
                # origin fetch above
                # ($_WT_ORIGIN_FETCH_RESULT) gives no independent confirmation
                # either. Silently proceeding here IS the #7765 defect - refuse
                # rather than guess "safe".
                if [[ "$JSON_OUTPUT" == "true" ]]; then
                    echo '{"success": false, "error": "forge-check-unavailable", "issueNumber": '"$ISSUE_NUMBER"', "originFetch": "'"$_WT_ORIGIN_FETCH_RESULT"'"}' >&3
                else
                    print_error "Could not verify via the forge whether an open PR already exists for '$BRANCH_NAME' (gh unavailable, unauthenticated, or rate-limited) - refusing to create a same-named branch from $BASE_DISPLAY blind."
                    if [[ "$_WT_ORIGIN_FETCH_RESULT" == "fetch-failed" ]]; then
                        echo "  (the earlier 'git fetch origin $BRANCH_NAME' also failed - see above)"
                    fi
                    ...
                    echo "  manually first (git branch $BRANCH_NAME $BASE_REF) - worktree.sh will then reuse it."
                fi
                exit 1
                ;;
        esac
