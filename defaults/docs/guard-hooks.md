...
**Third refinement pass (#5216):** the redaction described in the first pass above (quoted `--body`/`-m`/`--title`/`--notes`/`--comment` values) has a documented safety floor — it declines to redact any quoted span containing `$(` or a backtick, so a command-substitution-smuggled destructive command still denies. But that floor also meant a **heredoc-wrapped** value (`--body "$(cat <<'EOF' ... EOF)"`, the idiom this repo's own agent instructions use for multi-line comments) was **never** redacted, regardless of what the heredoc body merely *quoted* as inert prose — a dangerous-command example documented inside one (e.g. in a backtick code span) tripped the catastrophic scan exactly as a live invocation would. The catastrophic scan (`ALWAYS_BLOCK_PATTERNS`), the SQL DDL check, and the rm-scope-outside-repo target extraction now additionally run over a **heredoc-body-masked** working copy (`mask_heredoc_bodies_selective()`, #5205), independent of the `$(`/backtick exclusion above. This is **selective**, not the plain heredoc masker `extract_write_targets()` uses: a heredoc body that is genuinely fed to an inner interpreter (`bash <<'EOF' ... EOF`, `... | sh`) is left **unmasked** and still visible to the scan, so a smuggled destructive command inside such a body still hard-denies — only a heredoc body that is merely quoted inside a recognized flag value is narrowed. A separate, broader ask-tier check (e.g. `guards.cloudCli`'s `aws s3 (rm|rb|cp|mv|sync|mb)`) is untouched by this pass and may still legitimately downgrade some of these same examples from "no longer a catastrophic deny" to "ask" — that is a different, intentionally out-of-scope mechanism, not a residual gap in this fix.
