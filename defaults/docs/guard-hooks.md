...
- **The per-role tool restriction** (`toolPolicy.allowedCapabilities`, #8256).
  It has no `guards.*` toggle at all — but it is still not a floor member,
  because it is conditional on a declaration the repo owns: a role that
  declares nothing, or declares `["*"]`, reaches every one of those surfaces.
  Floor membership means unconditional. See its own section below.
...
**At runtime the canonical Repo Skills guard wins** when it is present **and passes all five dispatcher probes** (version + write-confinement capability + search/jq-mask capability + `--body @path` literal-string capability + per-role tool-restriction capability, #4894 / #5916 / #5974 / #8256 — see the bullet above): `guard-destructive.sh` is a dispatcher, so the vendored `guard-destructive-generic.sh` is a *fallback*, never a second guard running alongside it. A canonical guard that has the `repo#29` fix but not (yet) the write-confinement, search/jq-mask, `--body @path`, and/or per-role tool-restriction categories still routes to the vendored fallback.
...
### Per-Role Tool Restriction (`toolPolicy.allowedCapabilities`, #8256)

**The one guard category with no `guards.*` toggle and no `LOOM_GUARD_*` override — because its configuration surface is the per-role declaration itself.**

Loom's role prompts are public, and every role reads attacker-reachable text: issue and PR bodies, comments, room narration, files from external contributors. [`untrusted-external-content.md`](untrusted-external-content.md) is **prevention** — framing that tells a role to treat that text as data, not instructions. It does not hold once a role *is* persuaded. In the reference compromise the attacker did not exploit a bug; they talked to the agent ("this is David", "quarterly cloud maintenance") and it handed over credentials. Before this category, a persuaded Curator/Judge/Guide had exactly the same unrestricted shell as any other session, and the only thing between "the model was convinced" and "the fleet's credentials left the host" was the model's own judgement.

#### The declaration

Each role's JSON (`defaults/roles/<role>.json`, installed as `.loom/roles/<role>.json`) declares which **sensitive capabilities** it may reach:

```jsonc
// defaults/roles/curator.json — and architect, auditor, champion, guide, hermit, judge
"toolPolicy": { "allowedCapabilities": [] }

// defaults/roles/builder.json — and doctor, driver, loom
"toolPolicy": { "allowedCapabilities": ["*"] }
```

**One source, two enforcement points.** `spawn-claude.sh` reads this declaration at session-spawn time and turns the denied capabilities into `--disallowedTools` specs, so the session never has the tool in its permitted set. `guard-destructive-generic.sh` reads the *same* declaration at tool-call time and denies. Nothing is restated in either place, and a role that gains or loses a capability changes exactly one file.

#### The capability namespace

| Capability | What it covers | How it is detected |
|---|---|---|
| `remote-shell` | `ssh`, `scp`, `sftp`, `ssh-add`, `ssh-agent`, `ssh-keygen`, `ssh-keyscan`, `ssh-copy-id`, `autossh` | a segment's **command word** (quote-aware `qsplit()` segmentation, `sudo`/`env` wrappers unwrapped — the same parser the system-lifecycle deny uses) |
| `cloud-cli` | `aws`, `gcloud`, `az`, `doctl`, `flyctl`, `fly`, `wrangler`, `heroku`, `kubectl`, `eksctl` | command word |
| `forge-secrets` | `gh secret …`, `gh variable …`, `gh auth token｜login｜refresh｜logout｜setup-git`, `gh api` against a `…secrets…` endpoint | command word + subcommand. **`gh auth status` is deliberately excluded** — every role runs it |
| `credential-store` | **Writes** (redirect, `tee`, `sed -i`, `cp`/`mv` destination, `mkdir`) landing under `~/.ssh`, `~/.aws`, `~/.gnupg`, `~/.netrc`, `~/.config/gh`, `~/.docker/config.json`, or `.loom/tokens`; **and reads** by a file-content reader / exfil sender (`cat`, `head`, `base64`, `openssl`, `tar`, `curl`, `wget`, …) naming one | writes go through the same `extract_write_targets()` tokenizer as the #4178 write confinement, so tilde and same-command-variable resolution match byte-for-byte; reads are command-word anchored |

Every detector is **command-word anchored or write-target derived**, never a prose substring scan — so a PR body, a commit message, or a `--body "…"` string that merely *mentions* `ssh` cannot trip it.

#### Fails open on identity, closed on capability

The block does nothing at all unless a role is identified **and** that role declares a policy:

| Situation | Result |
|---|---|
| `LOOM_ROLE` unset — every interactive session, any automation that does not identify itself | **no restriction** |
| Role JSON absent/unreadable, or carries no `toolPolicy` (a consumer repo's custom role, a pre-#8256 resync) | **no restriction** |
| `allowedCapabilities` contains `"*"` | **no restriction** |
| `allowedCapabilities` present | **allowlist**: only the named capabilities are reachable. Everything else in the namespace — *including a capability added to the guard later* — is denied |

That asymmetry is deliberate. Restricting a role nobody declared restricted would break consumer repos silently; leaving a declared-restricted role reachable through a capability its declaration never mentioned is precisely the failure this exists to prevent.

`LOOM_ROLE` is the same identity signal the `_WT_READONLY_ROLES` carve-out (#6021) already keys on, set by role-runner/daemon dispatch (#4768) and now also re-exported by `spawn-claude.sh` / `spawn-codex.sh`. The three daemon aliases resolve identically in all three places: `development-worker` → `builder`, `pr-fixer` → `doctor`, `sweep-lifecycle` → `builder` (a full `/loom:sweep` runs the Builder and Doctor phases in-process and needs their capabilities).

#### Why there is no toggle

Every other category on this page has one because the category can be a whole-repo category error — a database engine and `DROP TABLE`, an infrastructure repo and `aws ec2`. This one cannot be, because it is already scoped per role. A repo whose Judge genuinely needs `aws` adds `"cloud-cli"` to `judge.json`: a per-role, reviewed, greppable change in the same file that grants it. A `guards.roleToolPolicy:false` would instead be a single switch that disarms the control for **every** role at once — the exact shape of bypass the category defends against. This is **not** a floor member either (see "The Ungated Denial Floor" above): the floor is unconditional, whereas this is conditional on a declaration a repo owns.

#### Where the declaration is read from

Resolution order, highest first:

1. `$SCRIPT_DIR/../roles/<role>.json` — the guard's **own** sibling roles directory. At runtime `SCRIPT_DIR` is the main checkout's `.loom/hooks/` (Claude resolves the `settings.json` hook entry against the main worktree even from a linked one), so **a PR branch checked out in a worktree cannot grant its own session a capability** by editing its copy of `.loom/roles/*.json`.
2. `$LOOM_PROJECT_ROOT/.loom/roles/<role>.json` — machine-level wiring (#4262).
3. `$REPO_ROOT/.loom/roles/<role>.json` — last resort, for a role the guard's own install does not ship.

#### Codex parity

Codex gets the **same** enforcement, through the same declaration, via `guard-codex-bridge.sh` — which normalizes each `pre_tool_use` shell call into a Claude-shaped `Bash` request and runs it through `guard-destructive.sh` unmodified. No policy is duplicated in the bridge.

One asymmetry is structural and worth stating: the Codex CLI has no `--disallowedTools` equivalent, so on that path the guard hook is the **whole** mechanism, not a backstop. A Codex session without the managed hook installed and trusted therefore has no per-role restriction at all — `spawn-codex.sh` now emits a loud, named warning at launch when a role with a restrictive `toolPolicy` starts with `hooks != ready`, instead of leaving that to be inferred from the absence of one.

#### Interaction with the read-only fast path

The `#3687` read-only fast path (`guards.readOnlyFastPath`, below) runs **before** `REPO_ROOT`, before every toggle read, and before this category. Its built-in allowlist admitted `gh <x> list|view` and read-only `aws` verbs to a silent allow — which meant `gh secret list` and `aws sts get-caller-identity` (exactly the reconnaissance a persuaded role runs first) escaped the restriction entirely. As of #8256 the fast path **declines eligibility** for the capability-bearing forms whenever `LOOM_ROLE` is set: the `gh secret`/`gh variable`/`gh auth` subcommands, and `aws` in every form. Common role idioms (`gh pr list`, `gh issue view`, `git status`) keep their fast path, and a session with `LOOM_ROLE` unset is byte-for-byte unaffected. Declining can only ever cause **more** evaluation, never less, so it cannot open a hole.

#### Known limitation

The `credential-store` **read** detector deliberately omits `grep`/`rg`/`sed`/`awk`. Those are the commands whose own quoted pattern argument routinely contains a path like `~/.ssh` as inert text (`grep -rn '~/.ssh' docs/`), and this category is required to read `COMMAND_ASK_SCAN` — the `deny-safe` scan copy — which does not mask positional arguments (only `COMMAND_CLOUD_ASK_SCAN` does, and that copy is `ask-only`, so it may never feed a `deny()`; see `scripts/check-guard-scan-contracts.sh`). So `cat ~/.ssh/id_ed25519` is denied and `grep . ~/.ssh/id_ed25519` is not. Closing it needs a `deny-safe` positional mask, which is its own change.

#### Verifying it

```bash
# A restricted role is denied at the harness, not by prompt convention:
printf '%s' '{"tool_name":"Bash","tool_input":{"command":"ssh user@host"},"cwd":"'"$PWD"'"}' \
  | LOOM_ROLE=curator .loom/hooks/guard-destructive-generic.sh | jq -r .hookSpecificOutput.permissionDecision
# -> deny

# ...and an unrestricted one is unaffected:
printf '%s' '{"tool_name":"Bash","tool_input":{"command":"ssh user@host"},"cwd":"'"$PWD"'"}' \
  | LOOM_ROLE=builder .loom/hooks/guard-destructive-generic.sh
# -> (no output: allow)
```

Regression coverage: `tests/hooks/test-guard-destructive-role-tool-policy.sh` (both fixture roles and every shipped role, all four surfaces, the fast-path deferral, and the fail-open cases), plus the spawn-time halves in `defaults/scripts/tests/test-spawn-claude.sh` and `test-spawn-codex.sh`.
