...
fields (installer-owned). **Exception (#5559):** the vendored `.loom/CLAUDE.md`'s
two-line `**Loom Version**` / `Last updated` header IS restamped on every
non-dry-run resync (a targeted field edit, same pattern as the `package.json`
decoy-version-field removal above, #4285) — this closes the drift where
`.loom/install-metadata.json`'s `loom_version` stayed current across resyncs
while the guide's own header silently fell behind, which in turn fooled
`scripts/install-loom.sh`'s metadata-preferring idempotency check into
reporting "already installed" without ever re-triggering the header's template
substitution. Everything else in `.loom/CLAUDE.md`, and all of `.loom/AGENTS.md`,
remains out of scope pending the managed-section-markers design (#4479).
