...
| `autonomous.workFinder.maxConcurrent` | **Live** — recomputed every work-finder tick as `min(disk headroom, ram headroom, configured maxConcurrent)` | `loom-daemon/src/work_finder.rs` module docs + "Dynamic concurrency scaling" in [`daemon-reference.md`](daemon-reference.md) |
...
| `autonomous.workFinder.maxAdmissionsPerTick` | **Requires a daemon restart** — resolved once at startup, deliberately mirroring `maxConcurrent`'s config read but not its live recompute | `loom-daemon/src/work_finder.rs` doc comment: "resolved once at startup" |
