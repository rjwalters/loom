...
| `autonomous.workFinder.quarantine.ttlSecs` | `LOOM_WORK_FINDER_QUARANTINE_TTL_SECS` | `3600` | How long a quarantine entry persists before auto-release. Zero/invalid → default. This is the **generation-1** TTL — a quarantine that relapses after release serves an escalated pause (vibesql#6639) |
| `autonomous.workFinder.quarantine.ttlMaxSecs` | `LOOM_WORK_FINDER_QUARANTINE_TTL_MAX_SECS` | `86400` | Ceiling on the escalated quarantine TTL (vibesql#6639): generation N serves `ttlSecs * 2^(N-1)` capped here, and a relapse re-quarantines on its FIRST further insta-crash (probation) rather than re-running the full threshold. A healthy outcome or `quarantine clear` resets the ladder. Zero/invalid → default; clamped up to `ttlSecs` |
