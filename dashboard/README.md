...
| `GET /api/spend` | none in-Worker — put behind Cloudflare Access | Authenticated query API: `ephemeral_compute` spend summed over a `since`/`until` window and bucketed by UTC day (issue #8306). |
...
| `GET /public/spend` | none (always public) | `{"withheld": true}` — no `ephemeral_compute` field survives redaction, so there is no reduced variant to serve (and a zeroed summary would read as a real idle window). |
