...
| Work queue | `#/` (section) and `#/queue` | From each host's `queue` entry (#8852 phase 3). The overview section gives per-host backlog / running / ready / blocked counts, the last work-finder tick, and a freshness badge that separates `idle` (recent tick, empty queue) from `stale` (no new tick in 15 min, counts dimmed and left out of the totals) and `no queue data` (the host never sent one). `#/queue` lists every issue across the fleet, folded by `repo#issue` so an issue two hosts see counts once under the host furthest along, in Running / Ready / Blocked tables with issue and PR links, host, phase and run time (joined from `activeSweeps`), time since the issue was created, and the daemon's blocking reason. Host detail adds that host's own queue in dispatch order. |
| Live status board | `#/live` | A board you can leave running on a second screen (#9077). It has headline tiles (hosts online, sweeps running, ready/blocked queue depth, peak token use, hosts needing attention) that flash when a value changes; one pill per host with a status dot, load meter and a heartbeat on each event it sends; one card per work item (`owner/repo#N`) with a curator → merge phase stepper, stopwatch timers that tick every second, the issue's and its PRs' current `loom:*` labels as chips, and the label transitions seen this session; and an activity ticker of live-tail events and label changes written as sentences. Cards never move once placed (#9094): new ones go last, a re-dispatched issue reuses its card, and a finished sweep's card stays in place, dimmed, until **Clear finished** (or the board passes 48 cards and it is the oldest idle one). Labels come from `labels.snapshot` records — on `/api/fleet-state` as `hosts[<id>].labels` and on the live tail — diffed client-side in `src/labelTracker.ts`; an issue whose labels change while you watch gets a card even with no sweep on it. It polls `/api/fleet-state` every 5s and pulls the next poll forward about 0.8s after any `sweep.*` event on `/api/events`. Unlike every other view, it patches keyed nodes in place instead of re-rendering, so its animations survive polls — see `src/views/liveBoard.ts`'s module doc. |
...
| `src/liveBoard.ts` / `src/livePanel.ts` / `src/views/liveBoard.ts` | The `#/live` status board (#9077): the DOM-free model (tiles, phase index, event sentences), the controller that owns the poll, one-second tick and SSE connection, and the keyed view. |
