...
2. **The v0.9.x spawn loop was removed.**
   `./.loom/scripts/spawn-loop.sh` worked through v0.10.x (emitting a
   stderr warning on every invocation referencing #3449) and was
   **removed in v0.11.0**. Migrate to
...
| Phase E | #3465 | `spawn-loop.sh` deprecation warning (removed in v0.11.0); doc-fiction rewrite across `CLAUDE.md` / `defaults/CLAUDE.md` / `defaults/roles/loom.md`; reconciliation in the prior migration guide | shipped |
...
**Symptom**: you had `LOOM_USE_SPAWN_LOOP=1 ./.loom/scripts/spawn-loop.sh start`
in your automation, in a tmux pane, or in a systemd unit. The loop
polled the forge for `loom:issue` items and detached one
`claude -p "/loom:sweep N"` child per ready issue.

**v0.10.x behaviour (historical)**: the script worked but was deprecated.
Phase E (#3465) added a deprecation warning that fired on every
`start` / `status` / `stop` invocation referencing #3449. The state file
(`.loom/spawn-loop-state.json`), the lock directory
(`.loom/locks/issue-<N>/`), and the per-child log files
(`.loom/logs/sweep-issue-<N>.log`) were all still produced during that
window.

**Removed in v0.11.0**: the script no longer exists. Migrate to the
daemon MCP surface.
...
rotation happens at the process-spawn boundary — same model as the
former spawn-loop, just driven by named operator dispatch instead of a
forge polling loop.

**Option 2: let `/loom:sweep` Stage -1 auto-detect the daemon**

If you previously typed `/loom:sweep 123` interactively while the
former spawn-loop ran in another window, you can now skip that entirely:
just keep `loom-daemon` running. The
...
**Suppressing the deprecation warning (historical, v0.10.x only):**

```bash
export LOOM_SUPPRESS_DEPRECATION=1
```

During the v0.10.x window this silenced the spawn-loop deprecation
warning. The script (and its warning) were removed in v0.11.0, so this
switch no longer has any effect for spawn-loop.
...
Deprecated throughout v0.10.x, then removed in v0.11.0:

- **`./.loom/scripts/spawn-loop.sh`** — the v0.9.x interim
  multi-account spawn loop (#3374). Every invocation emitted a
  stderr warning referencing #3449. The script remained functional
  through the entire v0.10.x cycle so downstream forks (sphere etc.)
  had one minor release of runway per the curator's risk note C on
  #3449, and was **removed in v0.11.0**.
  - Migration target: `mcp__loom__dispatch_sweep` against
    `loom-daemon`.
  - Suppression (v0.10.x only): `export LOOM_SUPPRESS_DEPRECATION=1`.
  - During the deprecation window the state file
    `.loom/spawn-loop-state.json` was produced and logs went to
    `.loom/logs/spawn-loop.log` and `.loom/logs/sweep-issue-<N>.log`.
...
- [ ] **File the sphere downstream coordination issue** per the #3382
      ...
      guide, and the v0.11.0 spawn-loop removal (shipped). The curator's
