...
| `loom-tools/src/loom_tools/orphan_recovery.py` | `.loom/spawn-loop-state.json::running[]` (issue, pid, last_heartbeat) + `gh issue list --label loom:building`. | `loom-orphan-recovery` / `recover-orphaned-shepherds.sh` — detects untracked `loom:building` issues and stale-heartbeat tasks whose PID is dead. | **ported (Phase 3.1.6, #3395)** | Daemon-state and progress-file reads removed. Daemon-era orphan types (`stale_task_id`, `invalid_task_id`) retired — spawn loop doesn't have task_ids. New `_pid_alive` guard prevents tear-down of SIGSTOPped sweeps. Test file trimmed (full deletion in Phase 3.3). |
...
|---|---|---|
| `defaults/scripts/recover-orphaned-shepherds.sh` | Operator-facing recovery. | **ported (Phase 3.1.6, #3395)** with `orphan_recovery.py` (Category 3). |
