#!/usr/bin/env bash
# clean.sh - Repository-root convenience wrapper for loom-clean
#
...
# Python `loom-clean` CLI). There is no Python fallback: PR #4301 (commit
# dba33666) deleted `loom_tools/clean.py` and its `loom-clean` console-script
# entry, and `loom-tools/` itself was removed by #4971. A daemon binary that
# predates the `clean` subcommand (a host mid-roll) fails loudly with a
# rebuild remedy instead of degrading into a stale-package error (#4384,
# #5739).
#
# Usage:
...
# shellcheck source=.loom/scripts/lib/locate-daemon-bin.sh
source "$SCRIPT_DIR/.loom/scripts/lib/locate-daemon-bin.sh"
