...
- **`uninstall-loom.sh`'s orphan-worktree `rm -rf` fallback no longer decides by parsing git's error text** (#6159, check-in on #5999) — the fallback that deletes a `.loom/worktrees/<entry>` git refuses to remove was gated solely on the substring `is not a working tree` appearing in `git worktree remove`'s stderr. git makes no compatibility promise about that wording, and it was the only thing standing between a live worktree and an unconditional recursive delete (the specific fear in Champion's merge hold on #5999). It now asks `git worktree list --porcelain` — a documented plumbing interface — whether the path is registered, and additionally requires the path to resolve strictly inside `.loom/worktrees/`; the `rm -rf` fires only when both hold. This is in the same spirit as the daemon-side guard (`should_force_remove_orphan_dir` in `loom-daemon/src/worktree_ops/clean.rs`, which likewise never gates its own `rm -rf` on the error string alone) but goes one step further: it does not read the error text at all, so it cannot be fooled by a rewording in either direction. Behavior under git 2.55 is unchanged for every orphan shape reachable today; what changes is that a future git rewording the message can no longer flip the classification in either direction. `defaults/scripts/tests/test-install-reinstall-safety.sh` Group 6 gains four cases, including one that pins git's actual error wording against the real binary (the string is still parsed by `defaults/scripts/worktree.sh` and `clean.rs`, so a git upgrade now breaks CI instead of silently changing field behavior) and one that runs the uninstall under a shim git whose message has been reworded.
