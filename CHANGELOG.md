...
**v1.0.0 is the breaking-changes release that closes out epic #3372 (shepherd/daemon deprecation).** The Python `loom-daemon` brain (`loom_tools/daemon_v2/`), the shepherd brain (`loom_tools/shepherd/`), and the `/shepherd` Claude Code slash command are **removed**. They were soft-deprecated in 0.9.x (see #3376) with a clear replacement narrative; this release deletes them. The replacements — `/loom:sweep <issue>` for single-issue lifecycle, `./.loom/scripts/spawn-loop.sh` for multi-issue batch orchestration, and GitHub Actions cron workflows under `.github/workflows/loom-*.yml` for support roles — were all shipped in 0.9.x and are unchanged.

See **[`docs/migration/v1.0.0-shepherd-deprecation.md`](docs/migration/v1.0.0-shepherd-deprecation.md)** for the authoritative migration guide, including per-CLI breaking changes, how to enable the replacements, and the three valid migration paths for downstream consumers.

### Breaking changes

- **Removed the Python `loom-daemon` brain** (`loom_tools/daemon_v2/`, ~4.2k LOC), the shepherd brain (`loom_tools/shepherd/`, ~16.5k LOC), `loom_tools/daemon.py`, `loom_tools/milestones.py`, `loom_tools/validate_state.py`, `loom_tools/test_failure_analysis.py`, `loom_tools/common/state.py::read_daemon_state`/`read_progress_files`/`find_progress_for_issue`, `loom_tools/common/systematic_failure.py`, `loom_tools/common/deprecation.py`, `loom_tools/models/daemon_state.py`, and `loom_tools/models/progress.py`. The forge-query half of `loom_tools/snapshot.py` was extracted to `forge_snapshot.py` first; only the daemon-state-coupled half was deleted. **Migration**: switch autonomous orchestration to `LOOM_USE_SPAWN_LOOP=1 ./.loom/scripts/spawn-loop.sh start` (Phase 1, #3374) and enable the GitHub Actions workflows under `.github/workflows/loom-*.yml` (Phase 2a, #3375) for support roles. See [the migration guide](docs/migration/v1.0.0-shepherd-deprecation.md#how-to-enable-the-replacements) for the full setup. (epic #3372, Phase 3.2 / Phase 3.3)
- **Removed `loom-daemon`, `loom-shepherd`, and `loom-validate-state` entry points** from `loom-tools/pyproject.toml`. Calling them by name now produces a "command not found" error. **Migration**: replace `loom-daemon` with `spawn-loop.sh` + GH Actions cron; replace `loom-shepherd` and `claude -p "/shepherd N"` with `claude -p "/loom:sweep N" --dangerously-skip-permissions`; `loom-validate-state` retires entirely (the validated file no longer exists — use a presence check on `.loom/spawn-loop-state.json` if you need a smoke test). See [the migration guide § Per-CLI breaking changes](docs/migration/v1.0.0-shepherd-deprecation.md#per-cli-breaking-changes). (epic #3372)
- **Removed the `/shepherd` Claude Code slash command** (`defaults/.claude/commands/loom/shepherd.md`), the supporting `shepherd-lifecycle.md`, the `loom-daemon` and `loom-shepherd` subagents (`defaults/.claude/agents/loom-{daemon,shepherd}.md`), and the role definitions they pointed at (`defaults/roles/loom.md`, `defaults/roles/shepherd.md`). **Migration**: use `/loom:sweep <issue>` for the same Curator → Builder → Judge → Doctor → Merge lifecycle. Mode C (#3384) is also available via `/loom:sweep --prs <pr-number-list>` for PR-set back-half workflows. If you have custom slash commands or hooks that referenced the deleted files, your next `install-loom.sh` will silently stop installing them — update those references. (epic #3372)
- **Renamed `loom-daemon-cleanup` → `loom-cleanup`** (#3396, Phase 3.1.7). The old name is gone. Update any cron entries, hooks, or scripts that referenced it by name.
- **Removed `.loom/daemon-state.json` and `.loom/progress/`** as supported state files. The producers are gone. Downstream code that reads either path will see "file not found" after v1.0.0. The replacements are `.loom/spawn-loop-state.json` (smaller, focused schema) and `.loom/sweep-checkpoint/issue-<N>.json` (per-issue checkpoint). **No compatibility shim is provided** — the rationale is documented in `docs/migration/daemon-state-consumers.md` §"Conclusion: what Phase 3 deletes vs preserves". The Rust daemon's `post_init.rs` `.gitignore` writer drops the patterns for the retired files (Phase 3.5) so a fresh install does not regenerate them.
- **Ported nine operator CLIs** (`loom-status` #3399, `loom-backlog` #3400, `loom-stuck-detection` #3401, `loom-completions` #3402, `loom-agent-metrics` #3404, `loom-orphan-recovery` #3414, `loom-cleanup` #3396/#3412, `loom-health-monitor` #3413, `loom-clean` #3415) to read `.loom/spawn-loop-state.json` + forge queries instead of `.loom/daemon-state.json` / `.loom/progress/`. Several CLIs lost specific fields that depended on state that no longer exists (e.g. `loom-status` lost `pipeline_state`, `warnings`, `total_prs_merged`, `last_*_trigger`; `loom-health-monitor` lost `stuck_agents` and `resource_availability` from its composite score; `loom-stuck-detection` lost the `missing_milestone` and `extended_work` indicators). See [the migration guide § Per-CLI breaking changes](docs/migration/v1.0.0-shepherd-deprecation.md#per-cli-breaking-changes) for the field-level diff per CLI.
- **Removed 22 daemon/shepherd-adjacent shell scripts** from `defaults/scripts/` (the authoritative list is in `docs/migration/daemon-state-consumers.md` §"Shell scripts to retire"). Six scripts were ported (`recover-orphaned-shepherds.sh`, `archive-logs.sh`, `check-active-session.sh`, `safe-worktree-cleanup.sh`, plus two docs-updates); the rest retire. The Rust-daemon control scripts (`start-daemon.sh`, `stop-daemon.sh`, `daemon-headless.sh`) are unaffected — they manage the Rust binary, not the deleted Python CLI.

### Added

- `docs/migration/v1.0.0-shepherd-deprecation.md` — single authoritative migration guide for the v1.0.0 breaking-changes release. Covers what was deleted, how to enable the replacements (spawn loop, GH Actions, `/loom:sweep`), per-CLI breaking changes, state-file consumer disposition, suppression of soft-deprecation warnings during the 0.9.x → 1.0.0 window, and three valid migration paths for downstream consumers. (Phase 3.6, #3403)

### Documentation

- Documentation sweep across `CLAUDE.md`, `defaults/CLAUDE.md`, `.loom/docs/`, `docs/guides/`, `docs/mcp/`, and READMEs to retire references to `/shepherd`, the Python `loom-daemon` CLI, `.loom/daemon-state.json`, and `.loom/progress/`. `.loom/docs/daemon-reference.md` is now a deprecated stub pointing at the spawn-loop replacements. Historical ADRs and historical CHANGELOG entries are intentionally not rewritten. (Phase 3.6, #3403)
