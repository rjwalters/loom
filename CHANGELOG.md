...
- **Opt-in per-worktree cargo target dirs, wired to the whole worktree lifecycle** (#8458, parent #8453 item 2) — on a host that points every checkout at one shared `build.target-dir`, that directory both grows without bound (**460 GB** on one fleet host) and **corrupts test verdicts**: Cargo uplifts the final binary to a single un-hashed `<target>/debug/loom-daemon` that whichever worktree built last overwrites, and integration tests execute exactly that path (#8453 records three incidents in one day, including a Judge run reporting 12 failures that passed 194/194 in isolation). Neither is fixable by sharing harder — a workspace crate's artifacts and incremental session are keyed by the crate's **absolute source path**, so two worktrees never share workspace-crate output in the first place; the shared dir only aggregates their garbage and collides their binaries. With `cargo.perWorktreeTargetDir` (env override `LOOM_PER_WORKTREE_TARGET_DIR`) enabled, `worktree.sh <N>` now gives each worktree **`<shared root>/wt/issue-<N>`**, records it in a gitignored `.loom-cargo-target-dir` marker **inside** the worktree, and `spawn-claude.sh` exports it as `CARGO_TARGET_DIR` for a claim-owning sweep — so the `cargo test` an agent runs itself is hermetic, which is where the false verdicts came from. Every removal path reclaims it: `worktree.sh remove`, `merge-pr.sh`'s post-merge cleanup, `loom-daemon clean`, and the daemon's periodic reaper (#4876, the path that catches a worktree whose PR merged on another host). `defaults/scripts/lib/cargo-target-dir.sh` remains the single source of truth for target-dir resolution — the marker is consulted *inside* its existing resolver, not beside it. Every rule lives in the daemon, per `.loom/docs/shell-language-policy.md`: `loom-daemon cargo-target-dir provision|path` is the creation half (`worktree_ops/cargo_target/provision.rs`) and `loom-daemon cargo-target-dir is-attributable|marker` answers the two removal-side predicates (`worktree_ops/cargo_target/per_worktree.rs`). What stays in bash is the *call sites* — two-line wrappers consulted from inside the existing resolver, so there is still exactly one resolution path.

  A **marker file** rather than #7239's `build.target-dir`-in-a-worktree-`.cargo/config.toml` because that vehicle is unavailable in a repo that *tracks* `.cargo/config.toml` (Loom itself does, for the CI-mirroring clippy `rustflags`): the redirect would leave every worktree with a modified tracked file, one `git add -A` from being committed. The marker is inside the worktree, so it is worktree-derived attribution evidence in exactly the sense #7239 requires, and it is not a Cargo config, so it touches no tracked state.

  **The #6013/#6014 binary-reuse fast path is preserved, not replaced** — that is the load-bearing constraint on this change, and the one way it could have caused a fleet-wide rebuild storm. `post-worktree.sh` still copies the main workspace's pre-built `loom-daemon` instead of rebuilding; it now resolves the **source** (the main workspace's own target dir, with a per-worktree `CARGO_TARGET_DIR` stripped — env beats config in Cargo, so honoring it there reports the binary "missing" on *every* worktree creation) and the **destination** (the marker / `LOOM_WORKTREE_CARGO_TARGET_DIR`) by different rules. A genuinely session-global `CARGO_TARGET_DIR` is still honored for the main workspace, unchanged. Both, plus the no-lib degraded path, are pinned by new cases in `tests/hooks/test-post-worktree-target-dir.sh`.

  Two attribution rules are relaxed, for these directories and only these: containment stops counting as sharing (the per-worktree dir lives *under* the shared root by design, so the primary checkout is always a containing "sharer" — without this nothing would ever be reclaimed), and a machine-global *source* no longer refuses a value carrying the per-worktree shape (the spawn path puts that value in exactly the `CARGO_TARGET_DIR` gate 2f guards against). Both are gated on a purely **structural** predicate — the path must end in `/wt/<the removed worktree's own directory name>` — so matching it is itself the attribution: a shared root can never satisfy it, and a corrupt or hand-edited marker can only ever name `<something>/wt/<this worktree's own name>`, never the shared root, a parent of it, or a sibling's dir. The #7239 data-loss guards are re-asserted with the feature ON in `defaults/scripts/tests/test-per-worktree-target-dir.sh` (38 cases) and `cargo_target/per_worktree.rs`.

  **Default OFF, deliberately.** Per-worktree dirs also stop sharing third-party `deps/`: nearly free when the host runs a `rustc-wrapper` (sccache keys on inputs, not on Cargo's path hash — the configuration #8453 measured), and a fleet-wide rebuild storm when nothing does, which *is* #6013/#6014. It is the operator's switch, on the same host where the shared `build.target-dir` was configured. On a host with **no** redirect it is a no-op even when enabled — `<worktree>/target` is already per-worktree and is removed with the worktree, so nothing is relocated. Does not retroactively clean directories already orphaned inside a shared root (a GC pass is #8459). Documented in `defaults/docs/troubleshooting.md` → "Per-worktree cargo target dirs".
